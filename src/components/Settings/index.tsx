...
export default function SettingsTab({
  ...
  // Since we only support L1 chains (Ethereum and Goerli), always show deadline settings
  ...
  // UniswapX removed - disable routing settings
  const uniswapXEnabled = false // chainId && isUniswapXSupportedChain(chainId)
  const showRoutingSettings = false // Boolean(uniswapXEnabled && !hideRoutingSettings)

  const isChainSupported = isSupportedChain(chainId)
  const Settings = useMemo(
    () => (
      <>
        {/* UniswapX routing settings removed */}
