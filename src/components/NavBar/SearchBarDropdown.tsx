...
export const SearchBarDropdown = (props: SearchBarDropdownProps) => {
  ...
  const showChainComingSoonBadge = chainId && (BACKEND_NOT_YET_SUPPORTED_CHAIN_IDS as readonly number[]).includes(chainId) && !isLoading
...
function SearchBarDropdownContents({
  ...
  // NFT trending collections disabled
  const trendingCollectionsAreLoading = false
  const formattedTrendingCollections: GenieCollection[] = []
  ...
  // Only count tokens, no NFT collections
  ...
  // NFT collections disabled, always show tokens only
  const showCollectionsFirst = false
  ...
  return hasInput ? (
    // Only show token search results, no NFT collections
    <Column gap="20">
      {tokenSearchResults}
    </Column>
  ) : (
    // Recent Searches and Trending Tokens only (no NFT collections)
    <Column gap="20">
