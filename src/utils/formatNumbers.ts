...
function formatNumber({
  ...
  // Determine if this is a fiat/USDC price type that needs USDC prefix
  const isFiatType = type === NumberType.FiatTokenDetails ||
  ...
  if (!hardCodedInput) {
    formattedValue = new Intl.NumberFormat(locale, formatterOptions).format(input)
  } else if (hardCodedInput.hardcodedOutput) {
    ...
    formattedValue = (prefix ?? '') + new Intl.NumberFormat(locale, formatterOptions).format(hardCodedInputValue)
  }

  // Add USDC prefix for fiat types (showing USDC value instead of USD)
  return isFiatType ? 'USDC ' + formattedValue : formattedValue
}
